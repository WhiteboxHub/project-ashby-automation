    ...
    * MANUAL     – blocks at every checkpoint for explicit approval.
    ...
    def show_browser_overlay(
        ...
        *,
        ...
        """Disabled: no banner overlay in Chrome."""
        self.clear_browser_overlay()

    def clear_browser_overlay(self) -> None:
        """Remove any banner overlay elements from Chrome."""
        try:
            self.page.evaluate(r"""() => {
                const els = document.querySelectorAll('[data-jobcli], [id*="jobcli"], [class*="jobcli"]');
